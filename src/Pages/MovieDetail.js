...
export const MovieDetail = () => {
   ...
   useEffect(()=>{
    async function fetchMovies() {
        const response= await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=fe5596c50d58e9e3e36bb2a2c0ee8e7c`)
         ...
         <section className="flex justify-around flex-wrap py-5">
            ...
            <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
                ...
                <div className="flex items-center">
                    ...
                    <p className="ms-2 text-xl font-bold text-gray-900 dark:text-white">{movie.vote_average}</p>
                    <span className="w-1 h-1 mx-1.5 bg-gray-500 rounded-full dark:bg-gray-400"></span>
                    <p  className="text-xl font-medium text-gray-900 underline hover:no-underline dark:text-white">{movie.vote_count} reviews</p>
                </div>
                <p className="my-4">Runtime: {movie.runtime} min.</p>
                <p className="my-4">Budget: {movie.budget}</p>
                <p className="my-4">Revenue: {movie.revenue}</p>
                <p className="my-4">Release Date: {movie.release_date}</p>
                <p className="my-4">IMDB Code: {movie.imdb_id}</p>
            </div>
